...
xlim([datetime(2021,1,1) datetime+days(5)])
...
hold on
plot(dates,cases,'r-','linewidth',3)
hold off

ylabel('Global Daily New COVID Cases','color','red')
set(gca,'ycolor','red')
%set(gca,'yticklabels',[])
...
legend('Ratio of Tech/Energy Stocks *','Global COVID Cases **','location','north')

title('Tech and Energy Stocks vs COVID')

txt0=text(datetime(2021,8,1),50,sprintf('  Last: %s',datestr(date,'mmm dd')),'fontsize',15);
txt1=text(datetime(2021,8,1),35,'* Ratio = $VGT/$VDE','fontsize',15);
txt2=text(datetime(2021,8,1),20,'** 7 Day Moving Average','fontsize',15);

txt3=text(datetime(2021,1,5),20,'@peterdevietien','fontsize',15);

%%
pngname=sprintf('~/projects/stock/post/tech_energy_cases');
print(pngname,'-dpng')
pngname=sprintf('~/projects/covid/post/omicron/tech_energy_cases');
print(pngname,'-dpng')